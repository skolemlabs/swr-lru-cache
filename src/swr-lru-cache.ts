...
{
  private readonly cache = new Map<string, State<Data, Error>>();
  ...
  public keys(): IterableIterator<string> {
    // We need to clone this iterator before returning it.
    //
    // This is because under the hood in some places, SWR will iterate over
    // these keys while calling `this.get()` in the loop body. `this.get()`
    // will mutate the underlying cache, which causes the iterator to loop
    // ~forever.
   ...
   */
  public get(key: string): State<Data, Error> | undefined {
   ...
   */
  public set(key: string, value: State<Data, Error>): void {
    this.setMostRecentlyUsed(key, value);
   ...
   */
  private setMostRecentlyUsed(key: string, value: State<Data, Error>): void {
    this.cache.delete(key);
